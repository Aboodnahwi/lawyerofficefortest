...
const LoginPage: React.FC<auth_page_props> = ({
  ...
  React.useEffect(() => {
    ...
    if (!is_online) {
      console.log("No internet connection available. Automatically entering with local data...");
      handle_offline_login();
      return;
    }
    ...
    check_supabase_schema().then((res) => {
      ...
      if (!res.success) {
        console.log("Supabase connection failed, entering offline mode automatically...");
        handle_offline_login();
      }
    });
  }, [current_mobile, initial_mode, is_online]);
  ...
  const handle_offline_login = () => {
    try {
      const cached_user_str =
      ...
      if (cached_user_str) {
        ...
        console.log("Entering offline mode with cached user profile.");
        on_login_success(cached_user, true);
        return;
      }
    } catch (e) {
      console.error("Error loading cached user for offline mode:", e);
    }

    const offline_user: any = {
      id: "offline_user",
      email: "offline@local.app",
      user_metadata: { full_name: "مستخدم بدون اتصال" },
    };
    on_login_success(offline_user, true);
  };
          ...
          {auth_step === "otp" ? (
            ...
            <form onSubmit={handle_auth} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-blue-600 text-white p-2.5 rounded-lg font-bold shadow-md hover:bg-blue-700 transition-all"
              >
                {loading
                  ? "جاري التحميل..."
                  : auth_step === "login"
