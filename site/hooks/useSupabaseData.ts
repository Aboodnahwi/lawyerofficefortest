...
export const useSupabaseData = (
  ...
  React.useEffect(() => {
    const load_local_data = async () => {
      ...
      try {
        ...
        let cached_data = await db.get(DATA_STORE_NAME, storage_key);
        if (!cached_data && user?.id) {
          cached_data = await db.get(DATA_STORE_NAME, APP_DATA_KEY_PREFIX);
        }
        ...
        let cached_deleted_ids = await db.get(
          DELETED_IDS_STORE_NAME,
          storage_key,
        );
        if (!cached_deleted_ids && user?.id) {
          cached_deleted_ids = await db.get(
            DELETED_IDS_STORE_NAME,
            APP_DATA_KEY_PREFIX,
          );
        }
  ...
  // Perform daily auto-backup if enabled
  React.useEffect(() => {
    if (!is_auto_backup_enabled || is_data_loading || !data) return;
    ...
    try {
      ...
      if (lastBackupDate !== todayStr) {
        const hasContent =
          ...
          (data.admin_tasks && data.admin_tasks.length > 0);
        ...
        if (hasContent) {
          ...
          console.log("Auto backup successfully generated for", todayStr);
        }
      }
    } catch (e) {
      console.error("Failed to perform auto backup:", e);
    }
