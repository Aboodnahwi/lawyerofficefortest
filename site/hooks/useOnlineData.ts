...
export const fetch_data_from_supabase = async (
    ...
    while (has_more) {
      ...
      while (table_attempt < table_max_retries) {
        try {
          ...
          console.warn(`Fetch table ${table} skipped due to error:`, err?.message || err);
          return all_data;
        }
      }
  ...
  while (attempt < max_retries) {
    try {
      ...
      console.warn(
        "fetch_data_from_supabase offline or connection skipped:",
        err?.message || err,
      );
      return {};
    }
        ...
        try {
          if (table !== "profiles") {
            try {
              ...
              if (log_error) {
                console.warn("sync_deletions insert warning:", log_error.message);
              }
            } catch (log_err) {
              console.warn("sync_deletions insert exception:", log_err);
            }
          }
          const { error } = await supabase
            .from(table)
          ...
          if (error) {
            console.warn(`Failed to delete records from ${table}:`, error.message);
          }
          break; // Success or warning handled
        } catch (err: any) {
          const message = String(err.message || "").toLowerCase();
          if (
            (message.includes("abort") ||
...
export const upsert_data_to_supabase = async (
  ...
  const { data: profile, error: profile_error } = await supabase
    ...
    .maybeSingle();
  if (profile_error) {
    console.warn("Could not fetch profile for upsert resolution:", profile_error.message);
  }
    ...
    while (attempt < max_retries) {
      try {
        ...
        if (error) {
          ...
          // Fallback to one-by-one for any non-network error in batch
          if (records.length > 1) {
            console.warn(
              `Batch upsert error for ${table}: ${error.message}. Attempting one-by-one fallback...`,
            );
            ...
            for (const rec of records) {
              try {
                ...
                console.warn(`Single record exception in ${table}:`, single_err);
              }
            }
            return successful_records;
          } else if (records.length === 1) {
            console.warn(`Single record upsert failed for ${table}:`, error.message);
            return [];
          }
          throw error;
        }
