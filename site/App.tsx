...
const App: React.FC<{ onRefresh: () => void }> = ({ onRefresh }) => {
  const [session, setSession] = useState<AuthSession | null>(() => {
    try {
      const cached =
        localStorage.getItem("lawyerAppLastUser") ||
        localStorage.getItem("lawyerAppLastUserData");
    ...
    return {
      user: {
        ...
        user_metadata: { full_name: "مستخدم محلي" },
      },
    } as any;
  });
  ...
  const [showBackupDownloadModal, setShowBackupDownloadModal] = useState(false);

  const supabase = get_supabase_client();
  const data = useSupabaseData(session?.user ?? null, isAuthLoading);

  // Check for daily backup prompt on login
  useEffect(() => {
    if (session?.user?.id && data.is_auto_backup_enabled && !data.is_data_loading) {
      const todayStr = new Date().toISOString().split("T")[0];
      const userKey = session.user.id;
      const lastBackupKey = `last_auto_backup_date_${userKey}`;
      
      const lastBackupDate = localStorage.getItem(lastBackupKey);
      if (lastBackupDate !== todayStr) {
        setShowBackupDownloadModal(true);
      }
    }
  }, [session?.user?.id, data.is_auto_backup_enabled, data.is_data_loading]);
  ...
  // Global Auth Watchdog: Don't let Auth hang on offline startup
  useEffect(() => {
    const isOffline = typeof navigator !== "undefined" && !navigator.onLine;
    const timeoutMs = isOffline || session ? 100 : 4000;
    const timer = setTimeout(() => {
      if (isAuthLoading) {
        console.warn("Auth check finished / timed out.");
    ...
    const {
      ...
      if (newSession) {
        ...
        if (event === "SIGNED_OUT") {
          ...
          console.log("Maintaining local/cached session for offline-first operation.");
          setSession((prev) => {
            if (prev) return prev;
            try {
              const cached =
  ...
  return (
    <DataProvider value={data}>
      <div className="flex flex-col h-screen print:h-auto bg-gray-50 print:bg-white">
        ...
        {showBackupDownloadModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white p-6 rounded-lg shadow-xl max-w-sm w-full space-y-4">
              <h3 className="text-lg font-bold text-gray-800">نسخة احتياطية يومية</h3>
              <p className="text-sm text-gray-600">هل تود تحميل نسخة احتياطية من بياناتك اليوم؟</p>
              <div className="flex gap-2">
                <button
                  onClick={() => {
                    data.export_data();
                    const todayStr = new Date().toISOString().split("T")[0];
                    localStorage.setItem(`last_auto_backup_date_${session?.user?.id}`, todayStr);
                    setShowBackupDownloadModal(false);
                  }}
