...
يرجى تقديم إجابات دقيقة ومهنية باللغة العربية. إذا سألك المستخدم عن بحث قانوني أو معلومة عامة، استخدم البحث في جوجل. إذا سألك عن الموكلين أو القضايا، اعتمد على البيانات المقدمة أعلاه.`;
  ...
  const handleSend = async () => {
    ...
    try {
      // الاستدعاء الآن يمر عبر خادمنا الخاص (api/gemini-chat)، والذي يحتفظ
      // بمفتاح Gemini بشكل آمن على الخادم فقط ولا يكشفه للمتصفح إطلاقًا.
      ...
      if (!res.ok) {
        setMessages((prev) => [
          ...
          {
            role: "model",
            text: data.error || "حدث خطأ غير متوقع. يرجى المحاولة مرة أخرى لاحقاً.",
          },
        ]);
        return;
      }
